...
#define all(x) x.begin(), x.end()
    ...
    inline int lower(const string &pattern) {
        int L = lower_bound(p.begin(), p.end(), pattern,
                            [&](int i, const string &pattern) { return s.substr(i, sz(pattern)) < pattern; }) -
                p.begin();
        return L;
    }
    ...
    inline int upper(const string &pattern) {
        int R = upper_bound(p.begin(), p.end(), pattern,
                            [&](const string &pattern, int i) { return s.substr(i, sz(pattern)) > pattern; }) -
                p.begin() - 1;
        return R;
    }
    ...
    inline string kth_substring(ll k, string s, vector<int> p) {
        ...
        while (k) {
            ...
            for (int i = piv; i < sz(pos) && k; i++) {
                if (cur[i] < sz(s) and c == s[cur[i]]) { // if the character is the same as the pivot
                    if (i == piv)
            ...
            if (!k) return s.substr(pos[piv], cur[piv] - pos[piv]);
            while (cur[piv] == n) piv++;
        }
        return "No such line.";
    }
