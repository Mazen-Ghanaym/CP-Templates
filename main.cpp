...
int LCS(int i, int j, int l)
{
    ...
    ret = 0;
    if (s[i] == t[j])
        ret = max(ret, 1 + LCS(i + 1, j + 1, fail[l][s[i] - 'A']));
    ret = max(ret, LCS(i + 1, j, l));
    ret = max(ret, LCS(i, j + 1, l));
    return ret;
}
string build(int i, int j, int l)
{
    if (l == k)
        return "";
    if (i == n || j == m)
        return "";
    int ret = dp[i][j][l];
    if (s[i] == t[j] && ret == 1 + LCS(i + 1, j + 1, fail[l][s[i] - 'A']))
    {
        return s[i] + build(i + 1, j + 1, fail[l][s[i] - 'A']);
    }
    if (ret == LCS(i + 1, j, l))
        return build(i + 1, j, l);
    return build(i, j + 1, l);
}
void solve(int tc)
{
    cin >> s >> t >> virus;
    n = sz(s), m = sz(t), k = sz(virus);
    memset(dp, -1, sizeof dp);
    fail = vector<vector<int>>(k + 1, vector<int>(26, 0));
    KMP kmp(virus);
    for (int i = 0; i < k; i++)
    {
        for (int j = 0; j < 26; j++)
        {
            fail[i][j] = kmp.failure(i, j + 'A');
        }
    }
    int lcs = LCS(0, 0, 0);
    if (lcs == 0)
