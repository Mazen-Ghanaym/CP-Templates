...
void solve(int tc) {
    ...
    for (int i = 1; i <= n; i++) cin >> nodes_val[i];
    for (int i = 2; i <= n; i++) {
        int u, v;
        cin >> u >> v;
        adj[u].push_back(v);
    ...
    // cout << nl;
    vector<int> idxs(n + 1);
    for (int i = 0; i < sz(euler_tour); i++) idxs[euler_tour[i]] = i + 1;
    vector<ll> euler_tour_val(sz(euler_tour));
    for (int i = 0; i < sz(euler_tour); i++) euler_tour_val[i] = nodes_val[euler_tour[i]];
