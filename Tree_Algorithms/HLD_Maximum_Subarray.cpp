...
template <typename T = int, int Base = 0> struct Segment_Tree {
    ...
    void push(int idx, int lx, int rx) {
        if (tree[idx].lazy == -100000) return;
    ...
    void update(const int l, const int r, const T v) { update(l, r, v, 1, 1, size); }
...
void rev(Node &a) { swap(a.pref, a.suff); }
template <typename T = int, const int VAL_ON_EDGE = 0> struct HLD {
    ...
    bool isUlower(int &u, int &v) {
        if (dep[root[u]] < dep[root[v]] || (root[u] == root[v] && dep[u] < dep[v])) return false;
        return true;
    }
    // make u lower
